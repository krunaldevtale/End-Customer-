...
(function() {
  function initChat() {
    ...
    // Helper: format time as "h:mmam/pm" (e.g. 11:40am)
    ...
    // Initial grouped bot message with current timestamp
    let messages = [
      { 
        type: 'bot', 
        html: `
          <div class="space-y-4 py-2">
            <div class="text-base">Is Cash on Delivery (COD) available?</div>
            <hr class="border-gray-200">
            <div class="text-base">How do I apply a coupon or discount code?</div>
            <hr class="border-gray-200">
            <div class="text-base">How do I get a refund if my order is cancelled?</div>
          </div>
        `,
        isGrouped: true,
        timestamp: new Date()
    ...
    function renderMessages() {
      ...
      messages.forEach(msg => {
        ...
        if (msg.type === 'bot') {
          ...
          const bubbleBg = msg.isGrouped ? 'bg-blue-50' : 'bg-gray-100';
          const bubblePadding = msg.isGrouped ? 'px-5 py-4' : 'px-4 py-2';
          const bubbleClass = `bubble-text-bot ${bubbleBg} rounded-xl ${bubblePadding} text-gray-800 max-w-md`;
          div.innerHTML = `
            <img src="/assets/img/Avatar.png" class="w-8 h-8 rounded-full object-cover" alt="Drodo">
            <div class="flex-1">
              <div class="text-sm font-semibold text-gray-800 mb-1">Drodo</div>
          ...
          div.innerHTML = `
            <div class="max-w-md">
              <div class="text-xs font-semibold text-gray-500 mb-1 text-right">You</div>
              <div class="bubble-text-user bg-sea-green-deep text-white rounded-xl px-4 py-2">
                ${escapeHtml(msg.text)}
              </div>
    ...
    function addBotMessage(textOrHtml, isHtml = false) {
      messages.push({ 
        type: 'bot', 
        text: isHtml ? null : textOrHtml,
        html: isHtml ? textOrHtml : null,
        isGrouped: false,
        timestamp: new Date()
      });
      renderMessages();
    ...
    function addUserMessage(text, attachmentHtml = null) {
      messages.push({ 
        type: 'user', 
        text, 
    ...
    function sendUserMessage(text, attachmentData = null) {
      ...
      addUserMessage(text || (attachmentData ? '📎 Sent an attachment' : ''), attachmentHtml);
      ...
      if (text && text.trim()) {
        ...
        setTimeout(() => {
          addBotMessage('Thank you for sharing the file. Our team will review it and get back to you soon.');
        }, 600);
      }
    }
